...
; 1) 规则（自上而下优先级）
; ==========================================================
; --- 192.168.11.123 专属 YouTube（必须位于其它域名规则之前） ---
; 同时满足：源设备为 192.168.11.123，且目标属于 YouTube
...
ruleset=↔️ teamviewer,[]GEOSITE,teamviewer
ruleset=↔️ teamviewer,[]GEOIP,teamviewer,no-resolve
...
custom_proxy_group=↔️ teamviewer`select`[]🇭🇰 HK`[]🇹🇼 TW`[]🇯🇵 JP`[]🇸🇬 SG`[]🇺🇸 US`[]🧊 冷门节点`[]🔵 SR 节点`[]⚪ 其它节点`[]🧭 手动选择`[]♻️ 自动选择`[]🌐 Default

; 192.168.11.123 的 YouTube 独立出口
...
; 原始节点名以 SR 开头；兼容自动添加在前面的旗帜、图标和空格
custom_proxy_group=🔵 SR 节点`select`(?i)^[^a-z0-9]*SR.*
; 除上述 SR 节点以外的所有节点，与 SR 组互斥
custom_proxy_group=⚪ 其它节点`select`(?i)^(?![^a-z0-9]*SR).*
